    ...
    <Sheet open={drawerOpen} onOpenChange={setDrawer}>
      <SheetContent className="flex w-full flex-col gap-0 bg-background p-0 sm:max-w-md">
        <SheetHeader className="border-b border-border/60 px-6 py-5 text-left">
          <SheetTitle className="font-serif text-2xl text-foreground">Your cart</SheetTitle>
          <SheetDescription>
            {items.length === 0
              ? "Your cart is feeling a little empty."
              : `${items.length} ${items.length === 1 ? "course" : "courses"} ready to bake.`}
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-4 px-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">Browse our courses and add a class you'd love to learn.</p>
              <Link
                to="/courses"
                onClick={closeDrawer}
            ...
            <ul className="divide-y divide-border/60">
              {items.map((c) => (
                <li key={c.id} className="flex gap-4 p-5">
                  <img src={c.image} alt={c.title} className="h-20 w-20 flex-shrink-0 rounded-xl object-cover" />
                  <div className="flex flex-1 flex-col">
                    <p className="text-xs uppercase tracking-wider text-primary">{c.difficulty}</p>
                    <p className="mt-0.5 font-serif text-base leading-tight text-foreground">{c.title}</p>
                    <p className="mt-1 text-xs text-muted-foreground">With {c.instructor.name}</p>
        ...
        {items.length > 0 && (
          <div className="border-t border-border/60 bg-muted/30 p-6">
            ...
            <p className="mt-3 text-center text-xs text-muted-foreground">
              Lifetime access · 30-day satisfaction guarantee
            </p>
          </div>
        )}
      </SheetContent>
